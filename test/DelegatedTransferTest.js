...
// Test that PYUSD operates correctly as a token with BetaDelegatedTransfer.
contract('BetaDelegatedTransfer PYUSD', function ([_, admin, owner, executor, recipient, whitelister, bystander]) {
  ...
  describe('as a token with a delegate whitelister', function () {
    ...
    it('cannot set whitelister to the same address', async function () {
      this.token.setBetaDelegateWhitelister(bystander, {from: owner})
      await assertRevert(
        this.token.setBetaDelegateWhitelister(bystander, {from: owner})
      );
    });
